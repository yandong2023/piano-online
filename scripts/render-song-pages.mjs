...
function renderSong(song, locale) {
  ...
  return `<!DOCTYPE html>
<html lang="${zh ? 'zh-CN' : 'en'}">
<head><base href="/">${sharedHead({ locale, title: pageTitle, description, canonical, alternate })}<script type="module" src="/js/main.js?v=${ASSET_VERSION}"></script><script type="module" src="/js/song-page.js?v=${ASSET_VERSION}"></script></head>
<body class="secondary-page song-detail-page" data-song-id="${esc(song.id)}">
${nav(locale)}
...
<footer class="footer"><div class="container"><div class="footer-content"><p>&copy; 2026 Piano Online</p><div class="footer-links"><a href="${zh ? '/songs/' : '/en/songs/'}">${zh ? '返回曲库' : 'Back to songs'}</a><a href="${languageUrl(song, locale)}">${zh ? 'English' : '中文'}</a></div></div></div></footer>
</body></html>`;
}
